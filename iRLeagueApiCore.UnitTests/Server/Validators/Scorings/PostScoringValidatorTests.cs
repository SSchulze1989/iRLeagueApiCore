...
namespace iRLeagueApiCore.UnitTests.Server.Validators.Scorings
{
    public class PostScoringValidatorTests : IClassFixture<DbTestFixture>
    {
        ...
        private static PostScoringRequest DefaultRequest(long leagueId)
        {
            return new PostScoringRequest(leagueId)
            {
        ...
        public async Task ValidateLeagueId(long leagueId, bool expectValid)
        {
            ...
            {
                var validator = new PostScoringValidator(dbContext);
        ...
        public async Task ValidateBasePoints(IEnumerable<double> points, bool expectValid)
        {
            using (var dbContext = fixture.CreateDbContext())
            {
                ...
                else
                {
                    ...
                    Assert.Contains(result.Errors, x => x.PropertyName == nameof(PostScoringRequest.BasePoints));
                }
            }
        }
        ...
        public async Task ValidateBonusPoints(IEnumerable<string> bonusPoints, bool expectValid)
        {
            ...
            {
                var validator = new PostScoringValidator(dbContest);
                var request = DefaultRequest(testLeagueId);
                ...
                {
                    Assert.False(result.IsValid);
                    Assert.Contains(result.Errors, x => x.PropertyName == nameof(PostScoringRequest.BonusPoints));
                }
            }
        }
        ...
        public async Task ValidateExtScoringSource(long? scoringId, bool useExtScoring, bool expectValid)
        {
            using (var dbContext = fixture.CreateDbContext())
            {
                ...
                {
                    Assert.False(result.IsValid);
                    Assert.Contains(result.Errors, x => x.PropertyName == nameof(PostScoringRequest.ExtScoringSourceId));
                }
            }
        }
    }
